 ...
 * صورة مكتبة إسلامية حقيقية + overlay نيلي عميق + زخرفة هندسية
 * الألوان: نيلي فاتح على خلفية داكنة في الهيرو
 */
...
export default function HeroSection({ stats, loading, searchQuery, onSearchChange }: HeroSectionProps) {
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full container py-14 flex flex-col items-center text-center">

        {/* ختم أكاديمي */}
        <div className="mb-5 flex items-center gap-3">
          <div className="h-px w-16 bg-amber-400/50" />
          <span
            className="text-amber-300 text-sm tracking-widest uppercase"
        ...
        >
          أكبر فهرس عربي لرسائل الدكتوراه والماجستير والبحوث العلمية
        </p>
        ...
        <div className="w-full max-w-2xl relative mb-10">
          ...
          <p className="text-indigo-300/70 text-xs mt-2" style={{ fontFamily: "Tajawal, sans-serif" }}>
  ...
  return (
    <div
      className={`rounded-xl p-4 text-center border transition-all hover:scale-105 ${
        accent
          ? "bg-amber-400/15 border-amber-400/30 hover:bg-amber-400/20"
    ...
    >
      ...
      <div
        className={`text-xl font-bold ${accent ? "text-amber-300" : "text-white"}`}
        style={{ fontFamily: "Tajawal, sans-serif" }}
      >
        {value}
      </div>
      <div
        className="text-indigo-200 text-xs mt-1"
        style={{ fontFamily: "Cairo, sans-serif" }}
      >
        {label}
      </div>
